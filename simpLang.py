...
class List(Value):
    ...
    def copy(self):
        copy = List(self.elements[:])  # copy the list
        copy.set_pos(self.pos_start, self.pos_end)
        copy.set_context(self.context)
        return copy
...
class Function(BaseFunction):
  ...
  class BuiltInFunction(BaseFunction):
    ...
    def no_execute_method(self, exec_ctx):
        raise Exception(f"No execute_{self.name} method defined")

    def copy(self):
        copy = self.__class__(self.name)  # safer way to instantiate
        copy.set_context(self.context)
        copy.set_pos(self.pos_start, self.pos_end)
        return copy
  ...
  BuiltInFunction.execute_print = execute_print  # add method to class

# Register the built-in function instance
...
class Interpreter:
  ...
  def visit_ForNode(self, node, context):
    ...
    start = res.register(self.visit(node.start_value_node, context))
    if res.should_return(): return res
    end = res.register(self.visit(node.end_value_node, context))
    if res.should_return(): return res
    step = res.register(self.visit(node.step_value_node, context)) if node.step_value_node else Number(1)
    if res.should_return(): return res
  ...
  def visit_CallNode(self, node, context):
    ...
    if hasattr(value_to_call, 'execute'):
        ...
        return res.failure(RTError(
            ...
            f"'{value_to_call}' is not callable",
            context
        ))
...
class BuiltInFunction:
    ...
    def execute_square(self, exec_ctx):
      value = exec_ctx.symbol_table.get('x')
      ...
      return RTResult().success(Number(value.value * value.value))
    execute_square.arg_names = ['x']
    ...
    def execute(self, args):
     ...
     if self.name == "print":
        ...
        print(output)  # still print on console
        return res.success(String(output))  # return the string to interface

     if self.name == "print_ret":
        output = ' '.join(str(arg) for arg in args)
        return res.success(String(output))

     return res.failure(RuntimeError(f"Unknown built-in function '{self.name}'"))
...
# Initialize built-in function instances
BuiltInFunction.print = BuiltInFunction("print")
BuiltInFunction.print_ret = BuiltInFunction("print_ret")
BuiltInFunction.input = BuiltInFunction("input")
BuiltInFunction.input_int = BuiltInFunction("input_int")
...
global_symbol_table.set("str", BuiltInFunction.str)  # ✅ Add this line
